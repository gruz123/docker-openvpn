...
* Initialize the `$OVPN_DATA` container that will hold the configuration files
  and certificates. The container will auto generate a passphrase to protect the
  private key used by the newly generated certificate authority. it's set your ip and port (1194/udp) automatically. You can always change it. 
...
* Generate a client certificate without a passphrase
      ...
      docker run -v $OVPN_DATA:/etc/openvpn --rm -it gruz123/ovpn easyrsa build-client-full CLIENTNAME nopass

* Retrieve the client configuration with embedded certificates

      docker run -v $OVPN_DATA:/etc/openvpn --rm gruz123/ovpn getclient CLIENTNAME > CLIENTNAME.ovpn
...
If you prefer to use `docker-compose` please refer to the [documentation](/README.md)

## Debugging Tips

* Create an environment variable with the name DEBUG and value of 1 to enable debug output (using "docker -e").

        docker run -v $OVPN_DATA:/etc/openvpn -p 1194:1194/udp --cap-add=NET_ADMIN -e DEBUG=1 gruz123/ovpn
...
Initialize the volume container using the `gruz123/ovpn` image with the
...
To generate a client certificate, `gruz123/ovpn` uses EasyRSA via the
`easyrsa` command in the container's path.  The `EASYRSA_*` environmental
variables place the PKI CA under `/etc/openvpn/pki`.

Conveniently, `gruz123/ovpn` comes with a script called `getclient`,
...
run time image (gruz123/ovpn) and the `$OVPN_DATA` data volume. To remove
...
## License
[![FOSSA Status](https://app.fossa.com/api/projects/git%2Bgithub.com%2Fgruz123%2Fdocker-openvpn.svg?type=large)](https://app.fossa.com/projects/git%2Bgithub.com%2Fgruz123%2Fdocker-openvpn?ref=badge_large)
