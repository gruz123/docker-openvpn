...
docker run -v $OVPN_DATA:/etc/openvpn --rm $IMG genconfig -u udp://$SERV_IP -2
...
docker run -v $OVPN_DATA:/etc/openvpn --rm -it $IMG otp_user $OTP_USER | tee $CLIENT_DIR/qrcode.txt
...
docker run -v $OVPN_DATA:/etc/openvpn --rm $IMG getclient $CLIENT | sed 's/auth-user-pass/auth-user-pass \/client\/credentials.txt/' | tee $CLIENT_DIR/config.ovpn
