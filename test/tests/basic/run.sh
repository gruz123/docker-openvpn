...
docker run -v $OVPN_DATA:/etc/openvpn --rm $IMG getclient $CLIENT | tee $CLIENT_DIR/config.ovpn

docker run -v $OVPN_DATA:/etc/openvpn --rm $IMG listclients | grep $CLIENT
